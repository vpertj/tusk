<script lang="ts">
  // 表单字段必须 $bindable：弹窗内 bind:value 只有绑定回父组件，
  // doConnect（父组件闭包）才能读到用户实际输入，否则发出的永远是父组件默认值
  let {
    showConnPanel,
    savedConns,
    connecting,
    dbType = $bindable('postgres'),
    sqlitePath = $bindable(''),
    host = $bindable('localhost'),
    port = $bindable(5432),
    user = $bindable(''),
    password = $bindable(''),
    dbname = $bindable('postgres'),
    sshEnabled = $bindable(false),
    sshHost = $bindable(''),
    sshPort = $bindable(22),
    sshUser = $bindable(''),
    sshPass = $bindable(''),
    connName = $bindable(''),
    saveConn = $bindable(true),
    pgHelp,
    PG_INSTALL_CMD,
    PG_START_CMD,
    close,
