// Tusk — PostgreSQL 驱动实现
// 连接、查询、编辑、DDL、同步、导出等全部 PG 能力；core 纯函数与 tauri command 解耦
use std::sync::Arc;
...
#[tauri::command]
pub async fn list_databases(state: State<'_, AppState>, conn_id: String) -> Result<Vec<DatabaseInfo>, String> {
    let entry = {
        let conns = state.conns.lock().await;
        conns.get(&conn_id).cloned().ok_or("连接不存在或已断开")?
    };
...
#[tauri::command]
pub async fn list_connections() -> Result<Vec<SavedConn>, String> {
